...
sayHelloButton.addEventListener('mouseout', resetBackground)
...
const repeatMyParam = () => {
    axios.get(`${baseURL}/repeat/whatever`)
        .then((res) => {
...
// CODE HERE
const customQuery = (event) => {
    axios.get(`${baseURL}/query-test?query`)
        .then((res) =>{
            console.log(res.data)
        })
}

document.getElementById('query-button').addEventListener('click', customQuery)
...
/*
    ...
    1 ANSWER: It sent a message saying I sent an empty query

    2: Send more than 1 query on the URL -- what happened? 
    2 ANSWER: It sent a message saying I sent more than 1 query
*/
...
function createFoodCard(food) {
    ...
    foodCard.innerHTML = `<h3>${food}</h3>`
      
    document.querySelector('body').appendChild(foodCard)
}
...
const foodContainer = document.querySelector('form')
